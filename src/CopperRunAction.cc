...
CopperRunAction::CopperRunAction():G4UserRunAction()
{
...
CopperRunAction::~CopperRunAction()
{}

void CopperRunAction::BeginOfRunAction(const G4Run* aRun)
{
  //inform the runManager to save random number seed
 // G4RunManager::GetRunManager()->SetRandomNumberStore(false);

  //initialize event cumulative quantities
  //CopperEventAction::Instance()->Reset();
}

void CopperRunAction::EndOfRunAction(const G4Run* aRun)
{
   G4int nofEvents = aRun->GetNumberOfEvent();


 /*  QMessageBox msgBox;
   msgBox.setText(QString::number(static_cast<double>(nofEvents)));
   msgBox.exec();*/
